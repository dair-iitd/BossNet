...
# Entity Word Drop
flags.DEFINE_float("word_drop_prob", 0.0,
                   "value to set, if word_drop is set to True")
flags.DEFINE_boolean('word_drop', True, 'if True, drop db words in story')

# PGen Loss
flags.DEFINE_float("p_gen_loss_weight", 1,
                   'relative weight to p_gen loss, > 1 gives more weight to p_gen loss')
flags.DEFINE_boolean("p_gen_loss", True,
                     'if True, uses additional p_gen loss during training')

# Model Type
flags.DEFINE_boolean(
    "hierarchy", True, "if True, uses hierarchy pointer attention")
flags.DEFINE_boolean(
    "rnn", True, "if True, uses bi-directional-rnn to encode, else Bag of Words")

# Output and Evaluation Specifications
flags.DEFINE_integer("evaluation_interval", 1,
                     "Evaluate and print results every x epochs")
flags.DEFINE_boolean("bleu_score", True,
                     'if True, uses BLUE word score to compute best model')
flags.DEFINE_boolean(
    "save", False, "if True, trains using previously saved model")
flags.DEFINE_boolean(
    "debug", False, 'if True, enables debug mode (Verbose Errors, but slower)')
...
# File Locations
flags.DEFINE_string("data_dir", "../data/dialog-bAbI-tasks/",
...
flags.DEFINE_string(
    "kb_file", "../data/dialog-bAbI-tasks/dialog-camrest-kb-all.txt", "kb file for this task")
...
def print_params(logging, args):
    ...
    logging.info('[{}] : {}'.format(
        'p_gen_loss_weight', args.p_gen_loss_weight))
